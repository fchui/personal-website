...
select OPTION in compose-setup database-password;
do
    case $OPTION in 
        compose-setup)
            ...
            break;;
        database-password)
            echo "Change the database password? y or n?";
            while true; do
                read -p $'' yn;
                case $yn in
                    [Yy]* ) read -s -p "New DB password:" dbPass; 
                        echo "${dbPass}" > ./db/password.txt; 
                        echo "New database password changed in db/password.txt"; break;;
                    [Nn]* ) break;; 
                    * ) echo "Please answer yes or no";;
                esac
            done
